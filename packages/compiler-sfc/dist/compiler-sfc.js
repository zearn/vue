    ...
    if (hasStaticWithDefaults()) {
      ...
      for (const m of members) {
        if (
          ...
          if (
            propsRuntimeDefaults.properties.some(p => p.key.name === m.key.name)
          ) {
            res +=
              m.key.name +
              (m.type === 'TSMethodSignature' ? '()' : '') +
...
const sass = (source, map, options) =>
  scss(
    source,
    map,
    Object.assign(Object.assign({}, options), { indentedSyntax: true })
  )
// .less
const less = (source, map, options) => {
  ...
  nodeLess.render(
    ...
    Object.assign(Object.assign({}, options), { syncImport: true }),
    (err, output) => {
      error = err
      result = output
    }
